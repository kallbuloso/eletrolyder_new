...
function getDeviceType(id) {
  const type = props.soDevicesType.find((item) => item.id === id)
...
<template layout="AppShell,AuthenticatedLayout">
  <template v-if="Number(props.count) > 0">
    <v-card class="mx-auto" :prepend-icon="pageIcon" :title="pageTitle">
      ...
      <v-card-text v-if="content.total > 10" class="d-flex">
        <v-row>
      ...
      <v-card-item>
        ...
        >
          ...
          <template #item.so_device_type_id="{ item }">
            {{ getDeviceType(item.so_device_type_id) }}
          </template>
